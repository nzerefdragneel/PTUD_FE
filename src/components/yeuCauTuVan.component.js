...
const YeuCauTuVan = () => {
    ...
    const handleYeuCauTuVan = async () => {
        ...
        if (ngayChon <= ngayHienTai) {
            setShowMessage(true);
            setnoiDungMessage('Vui lòng chọn ngày và giờ tư vấn sau thời điểm hiện tại!');
            return;
        }
        ...
        try {
            const response = await YeuCauTuVanService.DatLichTuVan(khachHang.iD_KhachHang, diaDiem, thoiGian);
    ...
    return (
        <div>
            ...
            <Button onClick={handleYeuCauTuVan} className="bg-blue-500 text-white px-4 py-2">
                GỬI YÊU CẦU
            </Button>
        </div>
    );
